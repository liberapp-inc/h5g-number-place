...
class Button extends GameObject{
    ...
    constructor( text:string, fontsize:number, fontRGB:number, xRatio:number, yRatio:number, wRatio:number, hRatio:number, rgb:number, alpha:number, line:boolean, onTap:()=>void, obj:any, id:number=0 ) {
        super();
        ...
        this.onTap = onTap;
        this.obj = (obj != null) ? obj : this;  // obj指定があればthis=objとしてコール、なければ(Button)thisからコール
        this.keyId = id;
        if( this.onTap ) this.display.addEventListener(egret.TouchEvent.TOUCH_TAP, this.onTap, this.obj);   // callback先のthisはobj
    ...
    onDestroy(){
        if( this.onTap ) this.display.removeEventListener(egret.TouchEvent.TOUCH_TAP, this.onTap, this.obj);
